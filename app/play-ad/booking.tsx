...
import { ArrowLeft, MapPin, Star, Info, X, Users, Eye, TrendingUp } from 'lucide-react-native';
...
export default function BookingScreen() {
  ...
  const handlePackageSelect = (pkg) => {
    if (pkg.type === 'custom') {
      Alert.alert(
        'Coming Soon!',
        'Custom packages will be available soon. You can create your own flexible advertising schedule with custom timing and duration.',
        [{ text: 'OK' }]
      );
      return;
    }
  ...
  const handleContinue = () => {
    if (selectedLocation && selectedPackage) {
      router.push({
        pathname: '/play-ad/business-details',
        params: {
  ...
  const getPackageTypeStyle = (type: string) => {
    switch (type) {
      case 'basic':
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity 
          ...
          onPress={() => router.replace('/play-ad/')}
          activeOpacity={0.7}
        >
          <ArrowLeft size={20} color={isDark ? '#FFFFFF' : '#111827'} />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>Select Location & Time</Text>
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {isLoading ? (
          ...
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No locations available. Please contact admin to add locations.</Text>
          </View>
        ) : (
          ...
          >
            ...
            <View style={styles.locationInfo}>
              ...
              <Text style={styles.priceText}>Starting from ₹{Math.min(...location.packages.filter(p => p.price > 0).map(p => p.price))}/package</Text>
              
              {selectedLocation?.id === location.id && selectedPackage && (
                <View style={styles.slotsContainer}>
                  <Text style={[styles.sectionTitle, { marginTop: 8, marginBottom: 8 }]}>
                    Selected Package
                  </Text>
                  <View style={[styles.packageCard, styles.selectedPackage]}>
                    <View style={styles.packageHeader}>
                      <Text style={styles.packageName}>{selectedPackage.name}</Text>
                      <Text style={[styles.packageType, getPackageTypeStyle(selectedPackage.type)]}>
                        {selectedPackage.type.toUpperCase()}
                      </Text>
                    </View>
                    <Text style={styles.packageTime}>{selectedPackage.time}</Text>
        ...
        <TouchableOpacity
          style={[
            ...
            { opacity: selectedLocation && selectedPackage ? 1 : 0.5 }
          ]}
          onPress={handleContinue}
          disabled={!selectedLocation || !selectedPackage}
        >
          <Text style={styles.continueButtonText}>Continue to Business Details</Text>
        </TouchableOpacity>
              ...
              {infoLocation && (
                <ScrollView showsVerticalScrollIndicator={false}>
                  <View style={styles.infoRow}>
                    <Users size={20} color="#10B981" style={styles.infoIcon} />
                    <View style={{ flex: 1 }}>
                ...
                {selectedLocation?.packages.map((pkg) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      pkg.type === 'custom' && styles.customPackageCard
                    ]}
